...
static void c_merge_block(emxArray_int32_T *idx, emxArray_real_T *x, int offset,
  int n, int preSortLevel, emxArray_int32_T *iwork, emxArray_real_T *xwork);
static void d_merge(emxArray_int32_T *idx, emxArray_real_T *x, int offset, int
                    np, int nq, emxArray_int32_T *iwork, emxArray_real_T *xwork);
  ...
  int n, int preSortLevel, emxArray_int32_T *iwork, emxArray_real_T *xwork)
{
  ...
  while (nPairs > 1) {
    ...
    nPairs >>= 1;
    for (nTail = 1; nTail <= nPairs; nTail++) {
      d_merge(idx, x, offset + (nTail - 1) * tailOffset, bLen, bLen, iwork,
              xwork);
    }
  ...
  if (nNonNaN > 1) {
    if (x->size[0] >= 256) {
      ...
      if (nBlocks > 0) {
        ...
        ib = nNonNaN - m;
        if (ib > 0) {
          c_merge_block(idx, b_x, m, ib, 2, iwork, xwork);
        }
    ...
    c_merge_block(idx, b_x, 0, nNonNaN, m, iwork, xwork);
  }
