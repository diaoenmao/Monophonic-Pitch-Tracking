 ...
 *                double Fs
 *                const double Parameter[33]
 *                emxArray_real_T *Pitch
 *                double *numfrms
 *                double *frmrate
 * Return Type  : void
 */
void yaapt(const emxArray_real_T *Data, double Fs, const double Parameter[33],
           emxArray_real_T *Pitch, double *numfrms, double *frmrate)
{
  ...
  for (i = 0; i <= numframes; i++) {
    if (DataC->data[i] <= Parameter[9]) {
      ...
      if ((1.0 <= y) || rtIsNaN(y)) {
        ...
        TPitch2->data[TPitch2->size[0] * i] = y;
      }
      ...
      if (2 > maxcands) {
        ...
        Trans = 1;
        b_Data_after = maxcands;
      }

      loop_ub = b_Data_after - Trans;
      ...
      y = TPitch2->data[TPitch2->size[0] * i];
      loop_ub = b_Data_after - Trans;
      for (b_Data_after = 0; b_Data_after < loop_ub; b_Data_after++) {
        TPitch2->data[(Trans + b_Data_after) + TPitch2->size[0] * i] = 1.0 - y;
      }
    }
  }
  ...
  emxInit_real_T(&r1, 2);
  c_rdivide(DataC, 5.0, r1);
  loop_ub = r1->size[1];
  for (Trans = 0; Trans < loop_ub; Trans++) {
    TPitch2->data[(maxcands + TPitch2->size[0] * Trans) - 3] = r1->data[r1->
      size[0] * Trans];
  }
  ...
  b_Data_after = 0;
  for (i = 0; i <= loop_ub; i++) {
    if (RPitch->data[(numcands + RPitch->size[0] * i) - 1] > 0.0) {
      b_Data_after++;
    }
  }
  ...
  r2->size[0] = 1;
  r2->size[1] = b_Data_after;
  emxEnsureCapacity((emxArray__common *)r2, Trans, (int)sizeof(int));
  b_Data_after = 0;
  for (i = 0; i <= loop_ub; i++) {
    if (RPitch->data[(numcands + RPitch->size[0] * i) - 1] > 0.0) {
      r2->data[b_Data_after] = i + 1;
      b_Data_after++;
    }
  }

  emxInit_real_T(&c_RPitch, 2);
  emxInit_real_T(&d_RPitch, 2);
  loop_ub = RPitch->size[1];
  b_Data_after = RPitch->size[0];
  Trans = d_RPitch->size[0] * d_RPitch->size[1];
  d_RPitch->size[0] = 1;
  d_RPitch->size[1] = loop_ub;
  emxEnsureCapacity((emxArray__common *)d_RPitch, Trans, (int)sizeof(double));
  for (Trans = 0; Trans < loop_ub; Trans++) {
    d_RPitch->data[d_RPitch->size[0] * Trans] = RPitch->data[(b_Data_after +
      RPitch->size[0] * Trans) - 2];
  }

  Trans = c_RPitch->size[0] * c_RPitch->size[1];
  c_RPitch->size[0] = 1;
  c_RPitch->size[1] = r2->size[1];
  emxEnsureCapacity((emxArray__common *)c_RPitch, Trans, (int)sizeof(double));
  loop_ub = r2->size[1];
  for (Trans = 0; Trans < loop_ub; Trans++) {
    c_RPitch->data[c_RPitch->size[0] * Trans] = d_RPitch->data[r2->data[r2->
      size[0] * Trans] - 1];
  }
    ...
    for (j = 0; j <= numcands; j++) {
      for (k = 0; k <= numcands; k++) {
        ...
        if (((RPitch->data[j + RPitch->size[0] * i] == 0.0) && (RPitch->data[k +
              ...
              (i - 1)] == 0.0))) {
          y = fabs(DataC->data[i - 1] - DataC->data[i]);
          if ((1.0 <= y) || rtIsNaN(y)) {
            d1 = 1.0;
          } else {
            d1 = y;
  ...
  Trans = r3->size[0] * r3->size[1];
  r3->size[0] = TPitch2->size[0];
  r3->size[1] = TPitch2->size[1];
  emxEnsureCapacity((emxArray__common *)r3, Trans, (int)sizeof(double));
  loop_ub = TPitch2->size[0] * TPitch2->size[1];
  for (Trans = 0; Trans < loop_ub; Trans++) {
    r3->data[Trans] = 1.0 - TPitch2->data[Trans];
  }

  emxFree_real_T(&TPitch2);
  path1(r3, b_Trans, Path_data, Path_size);
