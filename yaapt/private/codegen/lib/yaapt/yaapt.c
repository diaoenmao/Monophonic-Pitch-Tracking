           ...
           double *numfrms, double *frmrate)
{
  ...
  for (ixstart = 0; ixstart < (int)w[1]; ixstart++) {
    tempData->data[ixstart] = Data->data[ixstart] * Data->data[ixstart];
  }
  ...
  b_x->size[1] = loop_ub;
  emxEnsureCapacity((emxArray__common *)b_x, ix, (int)sizeof(creal_T));
  for (ix = 0; ix < loop_ub; ix++) {
    iy = ixstart - i0;
    for (numframes = 0; numframes < iy; numframes++) {
      b_x->data[numframes + b_x->size[0] * ix] = SpecData->data[(i0 + numframes)
        + SpecData->size[0] * ix];
    }
  }
  ...
  for (ix = 0; ix < loop_ub; ix++) {
    iy = ixstart - i0;
    for (numframes = 0; numframes < iy; numframes++) {
      b_SpecData->data[numframes + b_SpecData->size[0] * ix] = SpecData->data
        [(i0 + numframes) + SpecData->size[0] * ix];
    }
  }

  for (ix = 0; ix < 2; ix++) {
    w[ix] = b_SpecData->size[ix];
  }
  ...
  for (ixstart = 0; ixstart < i0; ixstart++) {
    c_y->data[ixstart] = rt_hypotd_snf(b_x->data[ixstart].re, b_x->data[ixstart]
      .im);
  }
  ...
  if ((c_y->size[0] == 0) || (c_y->size[1] == 0)) {
    ...
    for (i = 1; i <= c_y->size[1]; i++) {
      ...
      ix++;
      f0 = c_y->data[ixstart];
      for (ixstart = 2; ixstart <= c_y->size[0]; ixstart++) {
        ix++;
        f0 += c_y->data[ix];
      }
  ...
  emxInit_real_T(&pAvg, 2);
  emxInit_real_T(&pStd, 2);
  ...
  spec_trk(b_DataD, Fs, VUVEnergy, SPitch, tempData, pAvg, pStd);
  ...
  tm_trk(DataB, Fs, SPitch, pStd, TPitch1, TMerit1);

  /*   Pitch tracking from the filterd nonlinear signal  */
  tm_trk(DataD, Fs, SPitch, pStd, TPitch2, TMerit2);
  ...
  for (i = 0; i <= numframes; i++) {
    if (Energy->data[i] <= 0.1) {
      ...
      RPitch->data[iy + RPitch->size[0] * i] = 0.0;
      Merit->data[iy + Merit->size[0] * i] = 1.0 - Merit->data[Merit->size[0] *
        i];
      for (ixstart = 1; ixstart - 1 <= iy - 2; ixstart++) {
        if (RPitch->data[ixstart + RPitch->size[0] * i] == 0.0) {
          Merit->data[ixstart + Merit->size[0] * i] = 0.0;
        }
      }
    } else {
  ...
  for (i = 0; i <= numframes; i++) {
    RPitch->data[(iy + RPitch->size[0] * i) - 1] = tempData->data[tempData->
      size[0] * i];
    ...
    if (tempData->data[tempData->size[0] * i] > 0.0) {
      /*  voiced */
      Merit->data[(iy + Merit->size[0] * i) - 1] = Merit->data[Merit->size[0] *
        i];
    } else {
      /*  unvoiced */
      y = Energy->data[i] / 2.0;
      ...
      Merit->data[(iy + Merit->size[0] * i) - 1] = 1.0 - d0;
    }
  }
  ...
  for (i0 = 0; i0 < loop_ub; i0++) {
    RPitch->data[(iy + RPitch->size[0] * i0) - 2] = SPitch->data[SPitch->size[0]
      * i0];
  }

  emxFree_real_T(&SPitch);
  emxInit_real_T(&r1, 2);
  c_rdivide(Energy, 5.0, r1);
  loop_ub = r1->size[1];
  for (i0 = 0; i0 < loop_ub; i0++) {
    Merit->data[(iy + Merit->size[0] * i0) - 2] = r1->data[r1->size[0] * i0];
  }
  ...
  dynamic(RPitch, Merit, Energy, Pitch);
