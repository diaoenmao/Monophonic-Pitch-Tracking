           ...
           emxArray_real_T *Pitch, double *numfrms, double *frmrate)
{
  ...
  xoffset = r0->size[0] * r0->size[1];
  r0->size[0] = 1;
  r0->size[1] = (n - i0) + 1;
  emxEnsureCapacity((emxArray__common *)r0, xoffset, (int)sizeof(int));
  loop_ub = (n - i0) + 1;
  for (n = 0; n < loop_ub; n++) {
  ...
  N_F0_min = rt_roundd_snf(Parameter[2] / nFs * Parameter[4]);
  ...
  x->size[0] = n - i0;
  x->size[1] = loop_ub;
  emxEnsureCapacity((emxArray__common *)x, xoffset, (int)sizeof(creal_T));
  for (xoffset = 0; xoffset < loop_ub; xoffset++) {
    i = n - i0;
    for (k = 0; k < i; k++) {
      x->data[k + x->size[0] * xoffset] = SpecData->data[((i0 + k) +
        SpecData->size[0] * xoffset) - 1];
    }
  }
  ...
  emxEnsureCapacity((emxArray__common *)b_SpecData, xoffset, (int)sizeof(creal_T));
  for (xoffset = 0; xoffset < loop_ub; xoffset++) {
    i = n - i0;
    for (k = 0; k < i; k++) {
      b_SpecData->data[k + b_SpecData->size[0] * xoffset] = SpecData->data[((i0
        + k) + SpecData->size[0] * xoffset) - 1];
    }
  }

  for (xoffset = 0; xoffset < 2; xoffset++) {
    uv0[xoffset] = (unsigned int)b_SpecData->size[xoffset];
  }
  ...
  for (i0 = 0; i0 < loop_ub; i0++) {
    VUVEnergy->data[i0] = (FrmEnergy->data[i0] > Parameter[8]);
  }
  ...
  refine(y, TMerit1, TPitch2, TMerit2, Data_temp, FrmEnergy, VUVEnergy,
      ...
      for (k = 0; k <= numcands; k++) {
        ...
        if ((RPitch->data[j + RPitch->size[0] * i] > 0.0) && (RPitch->data[k +
          ...
          Trans->data[(k + Trans->size[0] * j) + Trans->size[0] * Trans->size[1]
            * i] = Parameter[28] * (fabs(RPitch->data[j + RPitch->size[0] * i] -
            RPitch->data[k + RPitch->size[0] * (i - 1)]) / fabs(RPitch->data[j +
        ...
        if (((RPitch->data[j + RPitch->size[0] * i] == 0.0) && (RPitch->data[k +
              ...
              (i - 1)] == 0.0))) {
          s = fabs(FrmEnergy->data[i - 1] - FrmEnergy->data[i]);
          if ((1.0 <= s) || rtIsNaN(s)) {
            d0 = 1.0;
          } else {
            d0 = s;
  ...
  for (i = 0; i <= numframes; i++) {
    Data_temp->data[i] = RPitch->data[((int)Path_data[i] + RPitch->size[0] * i)
      - 1];
  }
  ...
  *frmrate = Parameter[1];
  freqSelect(Data_temp, FrmEnergy, DataC);

  /*  Pitch = Pitch_Optimization(Pitch_Freq, Prm); */
  Smooth2(FrmEnergy, Parameter[32], Pitch);
