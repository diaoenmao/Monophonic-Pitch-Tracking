...
void Mymedfilt1(emxArray_real_T *s, emxArray_real_T *m)
{
  ...
  for (c_s = 0; c_s < 5; c_s++) {
    ...
    for (i12 = 0; i12 < c_s; i12++) {
      b_m->data[c_s + b_m->size[0] * i12] = d_s;
    }

    loop_ub = s->size[1];
    for (i12 = 0; i12 < loop_ub; i12++) {
      b_m->data[c_s + b_m->size[0] * (i12 + c_s)] = s->data[s->size[0] * i12];
    }

    loop_ub = 4 - c_s;
    for (i12 = 0; i12 < loop_ub; i12++) {
      b_m->data[c_s + b_m->size[0] * ((i12 + c_s) + s->size[1])] = e_s;
    }
  }
...
void b_Mymedfilt1(const emxArray_real_T *s, emxArray_real_T *m)
{
  ...
  for (s_idx_0 = 0; s_idx_0 < 5; s_idx_0++) {
    ...
    for (i14 = 0; i14 < s_idx_0; i14++) {
      b_m->data[s_idx_0 + b_m->size[0] * i14] = c_s;
    }

    loop_ub = b_s->size[1];
    for (i14 = 0; i14 < loop_ub; i14++) {
      b_m->data[s_idx_0 + b_m->size[0] * (i14 + s_idx_0)] = b_s->data[b_s->size
        [0] * i14];
    }

    loop_ub = 4 - s_idx_0;
    for (i14 = 0; i14 < loop_ub; i14++) {
      b_m->data[s_idx_0 + b_m->size[0] * ((i14 + s_idx_0) + b_s->size[1])] = d_s;
    }
  }
